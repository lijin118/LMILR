...
class CLASSIFIER:
    def __init__(self,model, _train_X, _train_Y, test_X, test_Y, seenclasses,novelclasses,
        ...
        self.test_X = test_X
        self.test_Y = test_Y
        # self.certain_seen_pred = certain_seen_pred
        # self.certain_seen_label = certain_seen_label
        ...
        if do_nothing==False:
            ...
            if test_only==False:
                if generalized:
                    self.acc_seen, self.acc_novel, self.H, self.output = self.fit()
                else:
    ...
    def fit_zsl(self):
        ...
        for epoch in range(self.nepoch):
            ...
            if self.train_only==False:
                with torch.no_grad():
                    output = self.model(self.test_X)  # .to(self.device)
                pred = torch.argmax(output.detach(), 1)
                acc = self.compute_per_class_acc(self.test_Y,pred,self.novelclasses.size(0))

            if acc > best_acc:
                best_acc = acc
                best_output = output
            print('[%.1f]  acc=%.4f' % (epoch, acc))
            self.loss = loss
        ...
        for epoch in range(self.nepoch):
            ...
            if self.train_only==False:
                with torch.no_grad():
                    output = self.model(self.test_X)  # .to(self.device)
                pred = torch.argmax(output.detach(), 1)
                acc_seen = self.compute_per_class_acc_gzsl(self.test_Y,pred,self.seenclasses)
                acc_novel = self.compute_per_class_acc_gzsl(self.test_Y,pred,self.novelclasses)
            ...
            print('[%.1f]     novel=%.4f, seen=%.4f, h=%.4f' % (epoch, acc_novel, acc_seen, H))
        self.loss = loss
        return best_seen, best_novel, best_H, best_output
